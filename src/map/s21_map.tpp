...
template <typename Key, typename T>
map<Key, T>::map(map&& m) noexcept : tree(m.tree) { m.tree.clear(); }
...
T& map<Key, T>::at(const Key& key) {
    ...
    if (!node) throw std::out_of_range("map::at");
...
template <typename Key, typename T>
typename map<Key, T>::iterator map<Key, T>::begin() { return iterator(tree.first(), tree.last()); }

template <typename Key, typename T>
typename map<Key, T>::iterator map<Key, T>::end() { return iterator(nullptr, tree.last()); }

template <typename Key, typename T>
typename map<Key, T>::const_iterator map<Key, T>::begin() const { return const_iterator(tree.first(), tree.last()); }

template <typename Key, typename T>
typename map<Key, T>::const_iterator map<Key, T>::end() const { return const_iterator(nullptr, tree.last()); }

template <typename Key, typename T>
bool map<Key, T>::empty() const { return !tree.get_size(); }

template <typename Key, typename T>
size_t map<Key, T>::size() const { return tree.get_size(); }

template <typename Key, typename T>
size_t map<Key, T>::max_size() const { return size_t(230584300921369395); }

template <typename Key, typename T>
void map<Key, T>::clear() { tree.clear(); }
...
std::pair<typename map<Key, T>::iterator, bool> map<Key, T>::insert_or_assign(const Key& key, const T& obj) {
    ...
    if (node) node->value.second = obj;
    else node = tree.insert_node(std::make_pair(key, obj));

    return std::make_pair(iterator(node, tree.last()), true);
}

template <typename Key, typename T>
void map<Key, T>::erase(iterator pos) { tree.remove_node(tree.find_node(pos->first)); }
...
template <typename Key, typename T>
bool map<Key, T>::contains(const Key& key) { return tree.find_node(key) != nullptr; }

//template <typename Key, typename T>
//typename map<Key, T>::Node* map<Key, T>::iterator::get_current() { return current; }

template <typename Key, typename T>
map<Key, T>::iterator::Iterator(Node<Key, T>* curr, Node<Key, T>* lst) : current(curr), last(lst) {}

template <typename Key, typename T>
map<Key, T>::iterator::Iterator(const Iterator &other) : current(other.current), last(other.last) {}

template <typename Key, typename T>
std::pair<const Key, T>& map<Key, T>::iterator::operator*() { return current ? current->value : last->value; }
...
typename map<Key, T>::iterator::Iterator &map<Key, T>::iterator::operator++() {
    if (current == last) current = nullptr;
...
template <typename Key, typename T>
typename map<Key, T>::iterator::Iterator &map<Key, T>::iterator::operator=(const Iterator &other) {
