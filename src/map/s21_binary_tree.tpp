...
template <typename Key, typename T>
b_tree<Key, T>::b_tree(const b_tree& other) : root(nullptr), size(0) {
  copy(other.root);
}
...
template <typename Key, typename T>
typename b_tree<Key, T>::b_tree& b_tree<Key, T>::operator=(
    const b_tree& t) noexcept {
  clear();
  copy(t.root);
  return *this;
}
...
Node<Key, T>* b_tree<Key, T>::insert_node(value_type v) {
  ...
  if (!root) {
    ...
    while (!(v.first > node->value.first && !node->right) &&
           !(v.first < node->value.first && !node->left)) {
...
template <typename Key, typename T>
size_t b_tree<Key, T>::get_size() const {
  return size;
}